...
def immediate_conversion(immediate):
    string_immediate = ' '.join([str(item) for item in immediate])
    return bin(int(string_immediate)).replace("0b", "")
...
for i in range (0, len(arr)-1):
    ...
    #This part will break up the instruction into opcode, destination register, source register(s), and shift amt
    while(instruction[j] != ' '):
        operation.append(instruction[j])
        j = j + 1
    print(operation)

    if (operation == ['A', 'D', 'D'] or operation == ['A', 'N', 'D']):
        ...
        print(MachineCodeLine)
        file2.write(MachineCodeLine + "\n")

    if (operation == ['S', 'T', 'U', 'R'] or operation == ['L', 'D', 'U', 'R']):
        print(operation_conversion(operation))
        j = j + 1
        while(instruction[j] != ' '):
            reg_destination.append(instruction[j])
        ...
        print(shamt_conversion(shamt))
        print(MachineCodeLine)
        MachineCodeLine = operation_conversion(operation)+register_conversion(reg_destination)+register_conversion(reg_source_1)+shamt_conversion(shamt)
        file2.write(MachineCodeLine + "\n")
    ...
    if operation == ['B', 'E', 'Q']:
        ...
        while(instruction[j] != ' '):
            immediate.append(instruction[j])
            j = j + 1
        print(immediate)
        print(immediate_conversion(immediate))
        
        MachineCodeLine = operation_conversion(operation)+register_conversion(reg_source_1)+register_conversion(reg_source_2)+immediate_conversion(immediate)
        print(MachineCodeLine)
        file2.write(MachineCodeLine + "\n")
    ...
    if operation == ['N', 'O', 'T']:
        ...
        j = j + 1
        print (instruction[j])
        MachineCodeLine = operation_conversion(operation)+register_conversion(reg_destination)+register_conversion(reg_source_1)
        file2.write(MachineCodeLine + "\n")
